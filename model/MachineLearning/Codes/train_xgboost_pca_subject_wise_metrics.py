...
def main():
    ...
    for data_type in DATA_TYPES:
        print(f"========== Processing DATA_TYPE: {data_type} ==========")
        
        # Initialize results dictionary
        all_results = {}
        
        # Main path now includes 'xgboost' and uses current data_type
        main_path = os.path.join(config.MAIN_SAVE_DIR, data_type, 'xgboost', f'PCA_{config.PCA_component}')
        
        for fold in config.Fold:
            all_results[fold] = {}
            
            for num_features in config.NUMBER_FEATURES:
                print(f'************************ DATA_TYPE {data_type} - Fold {fold} - Features {num_features} *****************************************')
                
                # Modified directory structure
                save_path = os.path.join(main_path, f'fold{fold}', f'features_{num_features}')
                os.makedirs(save_path, exist_ok=True)

                # Load selected features - updated for current data_type
                selected_features_path = os.path.join(config.MAIN_SAVE_DIR, data_type,
                    f'{data_type}_selected_features_{config.FEATURE}_{config.MAX_FEATURES}_std{config.Standard}.csv')
                selected_features_names = load_selected_features(selected_features_path, num_features)

                # Load and process data - updated for current data_type
                ...
                X_combined = clean_data(X_combined)  # Using clean_data instead of dropna for numpy array

                # Load annotations - updated for current data_type
                ...
                # Standardization - updated for current data_type
                if config.Standard:
                    scaler_path = os.path.join(config.MAIN_SAVE_DIR, data_type, f'scaler_fold{fold}.pkl')
                ...
                # Save results with PCA prefix and include data_type
                model_filename = f'{data_type}_PCA_xgb_model_{num_features}_features_{config.OVERSAMPLING}_GRIDSEARCH{config.GridSearch}.pkl'
                ...
                report_filename = f'{data_type}_PCA_classification_report_{num_features}_features_{config.OVERSAMPLING}_GRIDSEARCH{config.GridSearch}.txt'
                ...
                subject_results_df.to_csv(os.path.join(save_path, f'{data_type}_subject_level_results_{num_features}.csv'), index=False)
                ...
                plot_confusion_matrix(
                    chunk_cm, 
                    classes=[0, 1], 
                    save_path=os.path.join(save_path, f'{data_type}_PCA_chunk_confusion_matrix_{num_features}_features_{config.OVERSAMPLING}_GRIDSEARCH{config.GridSearch}.png'),
                    title=f'{data_type} Chunk-Level Confusion Matrix'
                )
                ...
                plot_confusion_matrix(
                    subject_cm, 
                    classes=[0, 1], 
                    save_path=os.path.join(save_path, f'{data_type}_PCA_subject_confusion_matrix_{num_features}_features_{config.OVERSAMPLING}_GRIDSEARCH{config.GridSearch}.png'),
                    title=f'{data_type} Subject-Level Confusion Matrix'
                )
                ...
                plt.title(f'{data_type} XGBoost Feature Importance Analysis - {num_features} Features')
                plt.tight_layout()
                plt.savefig(os.path.join(save_path, f'{data_type}_PCA_feature_importance_{num_features}_features_{config.OVERSAMPLING}_GRIDSEARCH{config.GridSearch}.png'))
                plt.close()
        ...
        # Generate separate summary tables for chunk and subject levels for current data_type
        # Chunk-level summary
        chunk_summary_rows = []
        ...
        chunk_summary_save_path = os.path.join(main_path, f'{data_type}_chunk_summary_results_xgboost_pca.csv')
        chunk_summary_table.to_csv(chunk_summary_save_path, index=False)
        print(f'Chunk-level summary results for {data_type} saved to {chunk_summary_save_path}')
        ...
        subject_summary_save_path = os.path.join(main_path, f'{data_type}_subject_summary_results_xgboost_pca.csv')
        subject_summary_table.to_csv(subject_summary_save_path, index=False)
        print(f'Subject-level summary results for {data_type} saved to {subject_summary_save_path}')

        # Create performance plots for both levels for current data_type
        # Chunk-level plot
        ...
        plt.title(f'{data_type} PCA Feature Selection Performance')
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(main_path, f'{data_type}_pca_performance_comparison.png'))
        plt.close()
        ...
        plt.xlabel('Number of Features')
        plt.ylabel('Mean Accuracy')
        plt.title(f'{data_type} PCA Feature Selection Performance (Chunk-level)')
        plt.grid(True)
        plt.savefig(os.path.join(main_path, f'{data_type}_pca_performance.png'))
        plt.close()
