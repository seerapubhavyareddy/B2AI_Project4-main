...
def main():
    ...
    for data_type in DATA_TYPES:
        ...
        # Create the main path for this data type
        main_path = os.path.join(config.MAIN_SAVE_DIR, data_type, f'PCA_{config.PCA_component}')
        ...
        for fold in config.Fold:
            ...
            for num_features in config.NUMBER_FEATURES:
                ...
                save_path = os.path.join(config.MAIN_SAVE_DIR, data_type, f'PCA_{config.PCA_component}', f'fold{fold}', f'features_{num_features}')
                os.makedirs(save_path, exist_ok=True)
                
                # Load selected features - update path for current data_type
                selected_features_path = os.path.join(main_path, f'{data_type}_selected_features_{config.FEATURE}_{config.MAX_FEATURES}_std{config.Standard}.csv')
                selected_features_names = load_selected_features(selected_features_path, num_features)
                
                # Load the combined data for current data_type
                ...
                # Clean the combined data first (fixed dropna issue - X_combined is numpy array)
                X_combined = clean_data(X_combined)
                
                # Update annotations dir path for current data_type
                ...
                if config.Standard:
                    # Standardize the datasets using the previously saved scaler - path updated for current data_type
        ...
        # Generate summary tables for both chunk and subject level for current data_type
        # Chunk-level summary
        ...
        # Save summary tables with data_type in filename
        chunk_summary_save_path = os.path.join(config.MAIN_SAVE_DIR, data_type, f'{data_type}_chunk_summary_results.csv')
        ...
        subject_summary_save_path = os.path.join(config.MAIN_SAVE_DIR, data_type, f'{data_type}_subject_summary_results.csv')
